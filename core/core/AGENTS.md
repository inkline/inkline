...
| Export                                                 | Shape                            | Notes                                                                                           |
| ------------------------------------------------------ | -------------------------------- | ----------------------------------------------------------------------------------------------- |
| `defineComponent`                                      | `(setup) \| (options, setup)`    | Sole component declaration entry point. `options` carries `slots`, `events`, `runtime`, `name`. |
...
None in this package today. Behavioral coverage lives in [`@inkline/compiler`](../compiler/) (compile + scenario tests assert what the _compiled_ output does). Adding direct unit tests here is rarely meaningful because the runtime is intentionally inert.
