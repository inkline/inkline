...
also carries every passed-through attribute and so is not the object you wrote. `INK0077`, an error,
...
> `INK0077` · error · The props object is read as a whole, not through a property
...
Only the capitalized `<Slot>` renders a slot. A lowercase `<slot>` is an ordinary host element, and
it is emitted verbatim into every target.
...
| INK0077 | error    | The props object is read as a whole. Only `props.<name>` is rewritten for each target.                                                  |
