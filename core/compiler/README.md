...
| `reportLevel`   | `"error" \| "warning" \| "info"`              | (see note)   | Minimum severity a diagnostic must be printed at. Read by `@inkline/cli` only — the pipeline always produces every diagnostic, so this changes what a build _reports_, never what it finds or what it exits with. Omitted, `compile` reports from `info` and `compile --watch` from `warning`. Also available as `--report-level` on the CLI. |
...
inkline compile "src/**/*.ink.tsx" --config inkline.config.ts --report-level warning
```

Flags: `--target`, `--src-dir`, `--out-dir` (default `dist`), `--source-map` (`external` | `inline` | `none`, default `external`), `--report-level` (`error` | `warning` | `info`), `--config`, `--clean` (default `true`), `--watch`, `--verbose`. `--target` is required unless the config file sets `targets`. CLI flags override config file values.

`--report-level` sets the floor for what is _printed_. It never changes what the compiler finds or
what the build exits with — the exit status is decided before the level applies. It defaults to
`info` for a one-shot compile and `warning` under `--watch`, where `info` notices would repeat on
every rebuild. When the level withholds something, the summary line says how much and names the
level to pass to see it.
