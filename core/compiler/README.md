...
4. **Declare each concern through one channel**, never silent precedence. Props come from
   ...
   the options `slots` map — one name in both is `INK0076`, a warning, and the options entry wins,
   since it is the only one that carries `required` and `scoped`.
...
| INK0076 | warning  | A slot name is declared twice. The options `slots` entry wins; the redundant declaration is reported.                                   |
