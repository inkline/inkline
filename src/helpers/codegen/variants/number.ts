 ...
 * @param variant
 */
export const codegenNumberVariant = (
    config: ResolvedConfiguration,
    name: string,
    variantName: string,
    variant: NumberPropertyVariant
): string => {
    const ctx = {
    ...
    Object.keys(variant).forEach((modifier) => {
        (numberModifiers[modifier] || numberModifiers[numberModifierAliases[modifier]])(
            ctx,
            variantName,
            variant[modifier] as string
        );
    });
