...
const classes = computed(() => Object.keys(props)
    .reduce((acc: Classes, property) => {
        if ((props as Record<string, string | number | boolean>)[property]) {
            acc[breakpointClass(`-${property}`, (props as Record<string, string | number | boolean>)[property] as string | number)] = true;
        }
