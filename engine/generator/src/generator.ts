...
export const generator = defineGenerator((configuration) => {
    ...
    if (options.variants && configuration.themes.default) {
        outputFiles.push({
            path: `variants.${options.typescript ? 'ts' : 'js'}`,
            content: consumeVariants(configuration.themes.default.variants, {
