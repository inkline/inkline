...
Runs on **port 6006**. Use `pnpm storybook` here or `pnpm run storybook:react` from the repo root (the latter co-runs the story compiler).

The `prestorybook` script runs `inkline compile stories` so the latest authoring source is reflected before Storybook starts.
