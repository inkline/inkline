...
11. **Destructuring the props object** — supported for plain, renamed, and defaulted bindings only. Parse consumes the statement and aliases each binding to its prop, so the emitted Solid source still reads through the reactive proxy and `requirePropsNotDestructured` still holds. A rest element, a nested pattern, or a computed key is INK0122; a binding naming an undeclared prop is INK0123. On React and Qwik an alias read inside a memo or an effect drops a default written in the pattern — read `props.x` there instead (`core/compiler/README.md`, "Two rules for the binding"). A props binding under any name other than `props` is a different defect one step earlier, and it is hard error INK0074 — raised on a **read** of the binding, matched on the symbol and not on the name, for the `defineProps` macro and the setup parameter alike. An unread binding (`const _props = defineProps<EmptyProps>()` in a headless component) stays legal.
