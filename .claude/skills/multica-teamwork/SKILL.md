...
5. **Verify.** Run your instructions' VERIFY block plus the issue's Verify section (`vp run ready` at the root, or the scoped equivalent, at minimum). Paste actual output evidence, not "tests pass". Mind the known noise baseline (`TS17004` fixtures) — see `inkline-project-map`. The expected INK0045/INK0068 notices are no longer part of that noise: both are `info` and the default `warning` floor withholds them, so reading them takes `--report-level info`.
