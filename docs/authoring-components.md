...
- **Destructure it only into plain, renamed, or defaulted bindings.** `const { label, size: dimension = "md" } = props` is read by the compiler, not emitted: each binding becomes an alias of the prop it names, and every read of the local compiles to the same output the matching `props.<prop>` read would. Solid therefore still reads through the reactive proxy — the `requirePropsNotDestructured` conformance invariant is about the emitted Solid source, and it still holds. A default written in the pattern becomes the prop's default on all seven targets; a default the prop already declares wins. A rest element, a nested pattern, and a computed key each name no single static prop, so no target can declare them — each is `INK0122`, an error. A binding naming an undeclared prop is `INK0123`.
