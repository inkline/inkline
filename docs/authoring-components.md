...
3. **`<Slot>` (imported from `@inkline/core`) renders a declared slot.** An unnamed `<Slot>` renders the default slot; `<Slot name="prefix" />` renders a named one. A lowercase `<slot>` is an ordinary DOM element, not a slot outlet — the compiler refuses it with `INK0076`. Declare each slot in the options object first. See [`core/compiler/README.md`](../core/compiler/README.md) → "Slots".
