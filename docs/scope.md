...
| **JSX spread attributes**                              | `<button {...props} />` is discarded by the parser and emits `INK0071`.                                                         | Per-target attribute binding needs every attribute name at build time; forwarding an unknown set needs a design first.           |
