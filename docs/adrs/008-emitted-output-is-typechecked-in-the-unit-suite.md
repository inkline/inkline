...
Deciders: QA · Informed by: internal tracker UXF-194, parent UXF-139, UXF-192 (the case that shipped uncaught)
...
Realised suite cost after the fix, `typecheck.test.ts` run in isolation: **67.97 s** and **72.49 s**
on two consecutive runs (115 tests — 80 swept fixtures, 25 skipped, 10 harness assertions). The whole
`@inkline/compiler` package is **60.64 s** wall for 223 files and 3164 tests; this file is the longest
single pole and is effectively the critical path under vitest's parallelism.

The pre-fix `typecheck.test.ts` took **227.96 s** for 420 tests — spawning `npx` 420 times to run a
decoy. So the working gate costs roughly **a third of what the broken one did**, and unlike the broken
one it asserts something. The honest way to state the price is not "it got cheaper": it is
**~68 s of CPU on the package's critical path, for a gate over 1 target and 80 fixtures.**
...
- The suite got **faster**: 227.96 s → 12.49 s for that file, because 420 subprocess spawns are gone.
...
- **Coverage is 1 target of 7 and 73 fixtures of 98.** This is a real gate over a narrow slice, not
  a comprehensive one. Anyone reading "emitted output is typechecked" without reading
  `typecheck-fixtures.ts` will overestimate it. That is why the scope lives in code, in a file the
  sweep imports, rather than in prose.
- **The 25-entry exclusion list is a maintenance surface**, and 13 of those entries are harness
  artifacts rather than bugs — noise that has to be re-read every time someone touches the list.
  Fixing them means teaching `compileFixture` to compile a fixture with its dependencies.
...
- ~12 s added to every `@inkline/compiler` unit run, on top of a 60 s package suite.
...
- **`compileFixture` learns to compile a fixture with its dependencies** → delete all 13 `[harness]`
  exclusions and re-measure. If the count does not drop to zero, the harness change is incomplete.
