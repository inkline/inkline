...
[ADR-010](./010-models-are-machine-written-from-definemodel.md) **in part**
...
> Decision 5 ("Option D is deferred, not rejected") is superseded: D was priced against the pinned
> TypeScript 7 toolchain on 2026-08-30 and found to have no plugin entry point anywhere in the
> package, so it is **closed on a measured blocker** with one named lift condition, not deferred.
> Decision 3 is **amended**: the `options.models` channel survives exactly as described, but the
> author no longer writes it — the compiler does, from the setup body's `defineModel` calls. Read
> "hand-maintained" in the Bad consequences as historical. **Decisions 1, 2, 4, 6 and 7 and every
> measured number below stand unchanged**, which is why this ADR's status remains `Accepted`.
